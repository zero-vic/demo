...
```c#
            ...
            var resObj = JsonConvert.DeserializeObject<CommonRequestResult>(res);
```

也可以直接封装一个HttpClient工具类
...
```C#
    ...
    public class HttpClientHelper : IDisposable
    {
        ...
        /// <returns></returns>
        public async Task<string> SendPost(string url, string data, ContentType contentType = ContentType.Json)
        {
            ...
            {
                throw new HttpRequestException($"POST request failed with status code: {resp.StatusCode}");
            }
        }
        /// <summary>
        /// 发送Post请求
        /// </summary>
        /// <typeparam name="T">响应类</typeparam>
        /// <param name="url">请求地址</param>
        /// <param name="requestData">请求数据</param>
        /// <param name="contentType">内容类型默认application/json</param>
        /// <param name="headers">请求头</param>
        /// <param name="timeout">超时时间</param>
        /// <returns></returns>
        public async Task<T> SendPost<T>(string url, object requestData, ContentType contentType = ContentType.Json, Dictionary<string, string> headers = null, TimeSpan? timeout = null)
        {
            string contentTypeValue = GetContentTypeValue(contentType);
            var content = new StringContent(JsonConvert.SerializeObject(requestData), Encoding.UTF8, contentTypeValue);
            ...
            {
                throw new JsonSerializationException("JSON serialization failed: " + ex.Message);
            }
            catch (JsonReaderException ex)
            {
                throw new JsonReaderException("JSON Parse Format Failed: " + ex.Message);
            }
        }
        /// <summary>
        /// 发送Get请求
        ...
        /// <returns></returns>
        public async Task<string> SendGet(string url, Dictionary<string, object> queryParams = null)
        {
        ...
        /// <returns></returns>
        public async Task<T> SendGet<T>(string url, Dictionary<string, object> queryParams = null)
        {
            try
            {
                url += BuildQueryString(queryParams);
        ...
        /// <returns></returns>
        public async Task SendDelete(string url, Dictionary<string, object> queryParams = null)
        {
            try
            {
            ...
            {
                throw new HttpRequestException("DELETE request failed: " + ex.Message);
            }
        }
        /// <summary>
        /// 参数拼接
        ...
        private string BuildQueryString(Dictionary<string, object> parameters)
        {
            ...
            var queryString ="?" + string.Join("&", parameters.Select(p => $"{Uri.EscapeDataString(p.Key)}={Uri.EscapeDataString(p.Value.ToString())}")).TrimEnd('&');
            return queryString;
        }
...
![image-20231204113312125](assets/image-20231204113312125.png)
...
```C#
   /// <summary>
    ...
    public class ExeclExportAttribute : Attribute
    {
        ...
        public ExeclExportAttribute(string columnName)
        {
...
```C#
        ...
        /// <returns></returns>
        public static string ExportExcel<T>(IEnumerable<T> entities, string fileName)
        {
            ...
            {
                ExeclExportAttribute execlExportAttribute = (ExeclExportAttribute)Attribute.GetCustomAttribute(property, typeof(ExeclExportAttribute));
                if (execlExportAttribute != null)
                {
                    var cell = headerRow.CreateCell(columnIndex1);
                    cell.SetCellValue(execlExportAttribute.ColumnName);
            ...
            var dateDataFormat = workBook.CreateDataFormat().GetFormat("yyyy-mm-dd"); // 设置日期格式
            cellStyle.Alignment = HorizontalAlignment.Center;
            cellStyle.VerticalAlignment = VerticalAlignment.Center;
            dateCellStyle.DataFormat = dateDataFormat;
            // 生成数据并计算列宽
            int rowIndex = 1;
                ...
                foreach (var property in properties)
                {
                    ...
                    if (execlExportAttribute != null)
                    {
                        ...
                        if (cellValue != null)
                        {
                            ...
                            {
                                DateTime detev;
                                DateTime.TryParse(cellValue.ToString(), out detev);
                                cell.CellStyle = dateCellStyle; // 应用日期单元格样式
                                cell.SetCellValue(detev);
                            }
                            else if (property.PropertyType == typeof(bool))
                            {
                                bool boolValue = false;
                                bool.TryParse(cellValue.ToString(), out boolValue);
                                cell.SetCellValue(boolValue);
                            }
                            else if (property.PropertyType == typeof(int))
                            {
                                int intValue = 0;
                                int.TryParse(cellValue.ToString(), out intValue);
                            ...
                            int cellValueLength = Encoding.Default.GetBytes(execlExportAttribute.ColumnName).Length;
            ...
            {
                sheet1.SetColumnWidth(i, (columnWidths[i] + 1) * 256); // 乘以256是以字符宽度为单位
            }

            string defaultPath = Directory.GetCurrentDirectory() + "/Download/output/temporaryFolder";
            if (!Directory.Exists(defaultPath))
            {
                Directory.CreateDirectory(defaultPath); // 不存在就创建文件夹
            }

            // 创建文件
            var saveFileName = DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + fileName + ".xlsx";
            var savePath = "/Download/output/temporaryFolder/" + saveFileName;
            using (var file = new FileStream(defaultPath + "/" + saveFileName, FileMode.Create))
            {
                workBook.Write(file);
            }
...
```c#
    ...
    public class RequistionListVo
    {
        ...
        /// </summary>
        [ExeclExport("采购单申请序号")]
        public string Cgdxh { set; get; }
        /// <summary>
        /// 采购单内容
        /// </summary>
        [ExeclExport("采购单内容")]
        public string Cgnr { set; get; }
        /// <summary>
        /// 关联项目
        /// </summary>
        [ExeclExport("关联项目")]
        public string Glxm { set; get; }
        /// <summary>
        /// 供应商名称
        /// </summary>
        [ExeclExport("供应商名称")]
        ...
        /// </summary>
        [ExeclExport("申请人员")]
        public string Sqrymc { set; get; }
        /// <summary>
        /// 审核状态
        /// </summary>
        [ExeclExport("审核状态")]
        public string Shzt { set; get; }
        /// <summary>
        /// 申请报价金额合计
        /// </summary>
        [ExeclExport("申请报价金额合计")]
        public string Sqbjhj { set; get; }
        /// <summary>
        /// 审核价合计
        /// </summary>
        [ExeclExport("审核价合计")]
        public string Shjhj { set; get; }
        /// <summary>
        /// 已报金额
        /// </summary>
        [ExeclExport("已报金额")]
        public string Ybje { set; get; }
        /// <summary>
        /// 剩余报销额度
        /// </summary>
        [ExeclExport("剩余报销额度")]
        public string  Sybxed { set; get; }
        /// <summary>
        /// 交货周期
        /// </summary>
        [ExeclExport("交货周期")]
        public int Jhzq { set; get; }
        /// <summary>
        /// 修改时间
        /// </summary>
        [ExeclExport("修改时间")]
        public string Xgsj { set; get; }
        /// <summary>
        /// 修改人
        /// </summary>
        [ExeclExport("修改人")]
        public string Xgrxm { set; get; }
    }
    
      var path = ExeclHelper.ExportExcel(list, "采购申请单");
