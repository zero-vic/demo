package com.hy.im.service.message.producer;
...
import com.hy.im.service.utils.UserSessionUtils;
...
//            rabbitTemplate.convertAndSend(RabbitConstants.MESSAGE_SERVICE_2_IM,session.getBrokerId()+"",msg);
    ...
    public boolean sendPack(String toId, Command command, Object msg, UserSession session){
        ...
        pack.setSiteId(session.getSiteId());
     ...
     * @param siteId
     */
    public List<ClientInfo> sendToUser(String toId, Command command, Object data, String siteId){
        List<UserSession> userSession
                = userSessionUtils.getUserSession(siteId, toId);
        List<ClientInfo> list = new ArrayList<>();
        for (UserSession session : userSession) {
            boolean b = sendPack(toId, command, data, session);
            if(b){
                list.add(new ClientInfo(session.getAppId(),session.getClientType(),session.getImei(),session.getSiteId()));
            }
        }
        return list;
    }
    ...
    public void sendToUser(String toId, Integer clientType,String imei, Command command,
                           Object data, Integer appId,String siteId){
        if(clientType != null && StrUtil.isNotBlank(imei)){
            ClientInfo clientInfo = new ClientInfo(appId, clientType, imei,siteId);
            sendToUserExceptClient(toId,command,data,clientInfo);
        }else{
            sendToUser(toId,command,data,siteId);
        }
    }
    ...
    public void sendToUser(String toId,Command command,Object data,ClientInfo clientInfo){
        UserSession userSession = userSessionUtils.getUserSession(clientInfo.getSiteId(), toId, clientInfo.getClientType(), clientInfo.getImei());
        if(userSession!=null){
    ...
    public void sendToUserExceptClient(String toId,Command command,Object data,ClientInfo clientInfo){
        List<UserSession> userSessions = userSessionUtils.getUserSession(clientInfo.getSiteId(), toId);
