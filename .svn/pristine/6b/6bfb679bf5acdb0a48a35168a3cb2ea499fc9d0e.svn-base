...
//        ZonedDateTime zdt = LocalDateTime.now().plusDays(3).atZone(zoneId);
        // 30分钟过期时间
        ZonedDateTime zdt = LocalDateTime.now().plusMinutes(30).atZone(zoneId);

        //添加构成JWT的参数
        JwtBuilder builder = Jwts.builder().setHeaderParam("typ", "JWT")
                .setId("restkeeper") //签发应用Id
                .setIssuedAt(Date.from(LocalDateTime.now().atZone(zoneId).toInstant())) //签发时间
     ...
     * @throws IOException
     */
    public static String createJWTByObj(Map<String, Object> tokenMap, String secret,LocalDateTime expiraTime) throws IOException {
        SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.HS256;
...
//        ZonedDateTime zdt = LocalDateTime.now().plusDays(3).atZone(zoneId);
        ...
        JwtBuilder builder = Jwts.builder().setHeaderParam("typ", "JWT")
                ...
                .setHeaderParam("alg", "HS256")  //加密算法
                .addClaims(tokenMap)
                .setExpiration(Date.from(expiraTime.atZone(zoneId).toInstant()))  //设置过期时间
                .signWith(signatureAlgorithm, secretKey);  //用密钥签名
